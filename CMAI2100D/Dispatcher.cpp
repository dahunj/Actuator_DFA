...
void CDispatcher::Get_JudgeReply(CString sModel, CString sLotId, CString sPortNo, CString sTrayNo, CString sCmNo, CString sBarcode, CString sCode, CString sJudge)
{
	...
	if (sJudge == "1") { gLot.nJudge_R[nPx][nTx][nCx] = gLot.nJudge_I[nPx][nTx][nCx][0] = 7; }	//치수불량1(7-FAI)
	if (sJudge == "2") { gLot.nJudge_R[nPx][nTx][nCx] = gLot.nJudge_I[nPx][nTx][nCx][0] = 8; }	//치수불량2(Gap)
	if (sJudge == "3") { gLot.nJudge_R[nPx][nTx][nCx] = gLot.nJudge_I[nPx][nTx][nCx][0] = 9; }	//치수불량3(Titl)
	if (gLot.nJudge_I[nPx][nTx][nCx][0] > 6) gLot.nRosJugCount[nPx][6]++;
...
void CDispatcher::Set_JudgeRequest(int nPortNo, int nTrayNo, int nCmNo)
{
	...
	if (pRosData->lstMdjIp.GetCount() < 1) 
	{
		...
		if (gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 7 || gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 8 || gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 9 || gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 10) 
		{
		...
		else
		{
			...
			gLot.nJudge_R[nPortNo-1][nTrayNo-1][nCmNo-1] = 4; gLot.nRosJugCount[nPortNo-1][2]++;	//ROS미연결-> ROS-Repair
		}
		CString strLog;
	...
	int		nNgNo      = gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0];
	gLot.nRosJugCount[nPortNo-1][0]++;
	...
	if (gData.bJahwa) {
		...
		strSendCmd.Format("JUDGE,REQUEST,%s,%s,%s,%d,%d,%d,%s,%s,%d,%d",
			pRosData->sHandlerIp, pEquipData->sModelName, strLotId, nPortNo, nTrayNo, nCmNo, strBarcode, strNgCode, gLot.nImageCnt[nPortNo-1][nTrayNo-1][nCmNo-1][1], gLot.nImageCnt[nPortNo-1][nTrayNo-1][nCmNo-1][0]);
	}
	...
	gLot.nJudge_R[nPortNo-1][nTrayNo-1][nCmNo-1] = 0;
...
BOOL CDispatcher::Is_JudgeDone(int nPortNo, int nTrayNo, int nCmNo)
{
	...
	if (nTerm > pEquipData->nTimeOver[1]) {		// Time Over
		CString strLog;
		strLog.Format("Judge Time Over (%d msec),Bar(%s),PortNo(%d),TrayNo(%d),CmNo(%d),SNo(%d), NG(%s)", pEquipData->nTimeOver[1], gLot.sBarCode[nPortNo-1][nTrayNo-1][nCmNo-1], nPortNo, nTrayNo, nCmNo, gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0], gLot.sNGCode_I[nPortNo-1][nTrayNo-1][nCmNo-1][0]);
		g_objLogFile.Save_DispatcherLog(strLog);
		...
		if (gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 7 || gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 8 || gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] == 9) {
			...
			gLot.nJudge_I[nPortNo-1][nTrayNo-1][nCmNo-1][0] = gLot.nJudge_R[nPortNo-1][nTrayNo-1][nCmNo-1] = 4;
			gLot.nRosJugCount[nPortNo-1][2]++;	//ROS TimeOver -> ROS-Repair
