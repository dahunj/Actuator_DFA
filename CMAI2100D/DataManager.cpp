...
BOOL CDataManager::Read_EquipData()
{
	...
	if (m_EquipData.nTimeOver[0] < 30000) m_EquipData.nTimeOver[0] = 30000;
...
void CDataManager::Read_NGTraySort()
{
	CString strKey, strData, strTemp;
	...
	for(int i=0; i<10; i++) {
		...
		int  x = 3;
		for(int j=0; j<4; j++) {
			AfxExtractSubString(strTemp, strData, j, ',');
			if		(strTemp == "A") m_EquipData.nTrayBase[i][x] = 11;	//허수불량
	...
	// 0:공백, 3:NG, ROS-NG, ROS재검, ROS Timeout, ROI_FAIL, 4(X)
	// 5:GRAB_FAIL, FAI_RECOFAIL_SP, Vision Result Timeout, MCBTM
