...
BOOL CCommon::Check_MainEmgAir()
{
	...
	if (!pDX20->iLightCurtain1 && gData.nMZDoorOpen[0] == 0) { Show_Error(91); return FALSE; }
	if (!pDX20->iLightCurtain2 && gData.nMZDoorOpen[1] == 0) { Show_Error(92); return FALSE; }
...
{	
	...
	int nNo = nPortNo;
	sLog.Format("[Common] Set_LotDataClear. PortNo[%d] LotID[%s] Count[%d]", nPortNo+1, gLot.sLotID[nNo], gLot.nCmCount[nNo]);
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 40; j++) {
			gMes.nCmResult[nNo][i][j] = gLot.nJudge_R[nNo][i][j] = gLot.nImageCnt[nNo][i][j][0] = gLot.nImageCnt[nNo][i][j][1] = gLot.nFOcapExist[nNo][i][j] = 0;
	...
	for (int i = 0; i <15; i++) gLot.nRosJugCount[nNo][i] = 0;
	...
	for (int k = 0; k < 40; k++) { gNG->sModuleID[nNo][k] = gNG->sHaimNGCd[nNo][k] = gNG->sCosmeticNG[nNo][k] = ""; gLot.nMarginal[nNo][k] = 0; }
	g_objDispatcher.Reset(nNo+1);
}

void CCommon::Set_LotDataCopy(int nFmPNo, int nToPNo)
{	
	CString sLog;
	sLog.Format("[Common] Set_LotDataCopy. PortNo[%d-%d] LotID[%s-%s] Count[%d-%d]", nFmPNo, nToPNo, gLot.sLotID[nFmPNo-1], gLot.sLotID[nToPNo-1], gLot.nCmCount[nFmPNo-1], gLot.nCmCount[nToPNo-1]);
	g_objLogFile.Save_HandlerLog(sLog);

	if (nFmPNo < 1 || nFmPNo > 30 || nToPNo < 1 || nToPNo > 30) return;
	int nFNo = nFmPNo-1;
	int nTNo = nToPNo-1;

	gLot.nCmCount[nTNo]			=	gLot.nCmCount[nFNo];
	gLot.nLotStatus[nTNo]		=	gLot.nLotStatus[nFNo];		
	gLot.nGoodCount[nTNo]		=	gLot.nGoodCount[nFNo];		
	gLot.nNgCount[nTNo]			=	gLot.nNgCount[nFNo];			
	gMes.nLotStatus[nTNo]		=	gMes.nLotStatus[nFNo];		
	gLot.nUnlGdTrayCount[nTNo]	=	gLot.nUnlGdTrayCount[nFNo];	
	gLot.nUnlNGTrayCount[nTNo]	=	gLot.nUnlNGTrayCount[nFNo];	
	gLot.nOutTrayCnt[nTNo][0]	=	gLot.nOutTrayCnt[nFNo][0];	
	gLot.nOutTrayCnt[nTNo][1]	=	gLot.nOutTrayCnt[nFNo][1];	
	gLot.nSkipCount[nTNo]		=	gLot.nSkipCount[nFNo];		
	gLot.nErrorCount[nTNo]		=	gLot.nErrorCount[nFNo];		
	gLot.dwErrorTime[nTNo]		=	gLot.dwErrorTime[nFNo];		
	gLot.dwStopTime[nTNo]		=	gLot.dwStopTime[nFNo];		
	gLot.sLotID[nTNo]			=	gLot.sLotID[nFNo];			
	gLot.sLLStartTime[nTNo]		=	gLot.sLLStartTime[nFNo];		
	gLot.sLLEndTime[nTNo]		=	gLot.sLLEndTime[nFNo];		
	gLot.sCarID_LD[nTNo]		=	gLot.sCarID_LD[nFNo];		
	gLot.sCarID_NG[nTNo]		=	gLot.sCarID_NG[nFNo];		
	gLot.sCarID_GD[nTNo]		=	gLot.sCarID_GD[nFNo];		
	gLot.sMZID_LD[nTNo]			=	gLot.sMZID_LD[nFNo];			
	gLot.sMZID_NG[nTNo]			=	gLot.sMZID_NG[nFNo];			
	gLot.sMZID_GD[nTNo]			=	gLot.sMZID_GD[nFNo];			
	gLot.nSlotNo_LD[nTNo]		=	gLot.nSlotNo_LD[nFNo];		
	gLot.nSlotNo_NG[nTNo]		=	gLot.nSlotNo_NG[nFNo];		
	gLot.nSlotNo_GD[nTNo]		=	gLot.nSlotNo_GD[nFNo];		
	gLot.nMZLastCar[nTNo]		=	gLot.nMZLastCar[nFNo];		
	gLot.bEmptyTray[nTNo]		=	gLot.bEmptyTray[nFNo];		
}
...
void CCommon::Set_LoadPickerDownMulti(int nPickNo, int nSub, int nCnt)
{
	DY_DATA_05 *pDY05 = g_objAJinAXL.Get_pDY05();
	DY_DATA_06 *pDY06 = g_objAJinAXL.Get_pDY06();
	if(nPickNo == 1)
	{
		if (nSub == 1) {
			if (nCnt > 0) {pDY05->oLoadPicker1Down1 = TRUE; pDY05->oLoadPicker1Up1 = FALSE;}
			if (nCnt > 1) {pDY05->oLoadPicker1Down2 = TRUE; pDY05->oLoadPicker1Up2 = FALSE;}
			if (nCnt > 2) {pDY05->oLoadPicker1Down3 = TRUE; pDY05->oLoadPicker1Up3 = FALSE;}
			if (nCnt > 3) {pDY05->oLoadPicker1Down4 = TRUE; pDY05->oLoadPicker1Up4 = FALSE;}
			if (nCnt > 4) {pDY05->oLoadPicker1Down5 = TRUE; pDY05->oLoadPicker1Up5 = FALSE;}
			if (nCnt > 5) {pDY05->oLoadPicker1Down6 = TRUE; pDY05->oLoadPicker1Up6 = FALSE;}
			if (nCnt > 6) {pDY05->oLoadPicker1Down7 = TRUE; pDY05->oLoadPicker1Up7 = FALSE;}
