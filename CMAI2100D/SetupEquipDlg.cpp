...
//	strData.Format("%d",	 gData.nRejectMaxCount); m_stcPitchData[2].SetWindowText(strData);
	strData.Format("%0.1lf", pEquipData->dEevatOffset); m_stcPitchData[2].SetWindowText(strData);
...
//	m_stcPitchData[2].GetWindowText(strData); nData = atoi(strData); INI.Set_Integer("OPTION", "REJECT_COUNT", nData);
	m_stcPitchData[2].GetWindowText(strData); dData = atof(strData); INI.Set_Double("OPTION", "ELEVATOR_OFFSET", dData, "%0.3lf");
...
void CSetupEquipDlg::Save_ModelEquipData(CString sPath)
{
	...
	if (!INI.Check_File()) {
		AfxMessageBox("Model Folder In EquipData.ini File Not Found!!");
		return;
	}
