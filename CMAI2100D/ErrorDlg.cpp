...
void CErrorDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
	...
	if (bShow) {
		...
		if (m_nErrNo == 7013 || m_nErrNo == 7513) strErrPick.Format(" #==> LotID[%s] TrayNo[%d] Module[%d]", gAlm.sAlmLID[0], gAlm.nAlmTNo[0], gAlm.nAlmPNo[0]);
		...
		if (m_nErrNo == 9004) strErrPick.Format(" #==> LotID[%s] Recipe[%s]", gMes.sHostLotID, gMes.sHostRecipe);
		...
		if (m_nErrNo == 9181) strErrPick.Format(" #==> [%s] 불양률이 [%0.1lf%%] 발생했습니다. (설정: %0.1lf%%). 연속 %d회 발생되었습니다.#      DFA공정에 내용전달 해주세요.# [%s]",
														gCap.sAlmDefectName, gCap.dAlmDefectPercent, gCap.dDefectPercent[0], gCap.nConsecutiveMZCnt[0], gCap.sAlmMZID);
		if (m_nErrNo == 9182) strErrPick.Format(" #==> [%s] 불양률이 [%0.1f%%] 발생했습니다. (설정: %0.1f%%). 연속 %d회 발생되었습니다.#      DFA공정에 내용전달 해주세요. AVI Master Sample 투입하여 Correlation 이상여부를 확인해 주세요.#      [%s]",
														gCap.sAlmDefectName, gCap.dAlmDefectPercent, gCap.dDefectPercent[0], gCap.nConsecutiveMZCnt[1], gCap.sAlmMZID);
	/*	if (m_nErrNo == 9183) strErrPick.Format(" #==> [%s] 불양률이 [%0.1f%%] 발생했습니다. (설정: %0.1f%%). 연속 %d회 발생되었습니다.#      DFA공정에 내용전달 해주세요. AVI Master Sample 투입하여 Correlation 이상여부를 확인해 주세요.#      [%s]",
		...
		if (m_nErrNo == 2803 || m_nErrNo == 7013 || m_nErrNo == 7513 || m_nErrNo == 9002 || m_nErrNo == 9011 || m_nErrNo == 9015 || m_nErrNo == 9016 || m_nErrNo == 9017) {
		...
		if (m_nErrNo == 4005 || m_nErrNo == 4006 || m_nErrNo == 4007 || 
			m_nErrNo == 4505 || m_nErrNo == 4506 || m_nErrNo == 4507 || 
			m_nErrNo == 8007 || m_nErrNo == 8207 || m_nErrNo == 8407 || m_nErrNo == 8607 ||
			m_nErrNo == 8008 || m_nErrNo == 8208 || m_nErrNo == 8408 || m_nErrNo == 8608 ||
		...
		strLog.Format("[Error Mode] ErrorNo:%s SeqNo:%d-%d-%d-%d-%d  %d-%d-%d-%d-%d  %d-%d-%d-%d-%d  %d-%d-%d-%d-%d  %d-%d-%d-%d-%d  %d-%d-%d-%d-%d  %d-%d-%d Version(%s)", strErrNo,
									...
									*(pCase+30), *(pCase+31), *(pCase+32), MAIN_VERSION);
...
{
	if (m_nErrNo == 2803) { if (g_objCommon.Show_MsgBox(2, "Magazine Skip 처리를 하시겠습니까?") != IDOK) return; }
	else				  { if (g_objCommon.Show_MsgBox(2, "Module Skip 처리를 하시겠습니까?") != IDOK) return; }
...
void CErrorDlg::OnBnClickedBtnErrRetry()
{
	...
	CString sLog;
	sLog.Format("[Error Mode] Retry button push]  (%d-%s)", gLot.nJobCycle, gLot.sLotID[gLot.nJobCycle-1]);
	g_objLogFile.Save_HandlerLog(sLog);
	ShowWindow(SW_HIDE);
}
...
//	if (m_nErrNo == 2118 || m_nErrNo == 2218 || m_nErrNo == 2318 || m_nErrNo == 2418 || m_nErrNo == 2518 || m_nErrNo == 2618 || m_nErrNo == 2718) {
	if (m_nErrNo == 2118 || m_nErrNo == 2119) {
		int nENo = 1;
	...
	if (m_nErrNo == 9001 || m_nErrNo == 9002 || m_nErrNo == 9004 || m_nErrNo == 9007 || m_nErrNo == 9008 || m_nErrNo == 9015 || m_nErrNo == 9009 ||
		m_nErrNo == 9021 || m_nErrNo == 9022 || m_nErrNo == 9023 || m_nErrNo == 9030 ||
		m_nErrNo == 4008 || m_nErrNo == 4009 ||
		m_nErrNo == 4508 || m_nErrNo == 4509 ) { 
		int nCase1 = g_objSequenceMain.Get_MainRunCase(AUTO_LOAD_STAGE_1);
		int nCase2 = g_objSequenceMain.Get_MainRunCase(AUTO_LOAD_STAGE_2);
		if (nCase1 > 5 && nCase1 < 10) g_objSequenceMain.Set_MainRunCase(AUTO_LOAD_STAGE_1, 6);
		if (nCase2 > 5 && nCase2 < 10) g_objSequenceMain.Set_MainRunCase(AUTO_LOAD_STAGE_2, 6);
	}
	...
	if (m_nErrNo == 4005 || m_nErrNo == 4006 || m_nErrNo == 4007 || m_nErrNo == 8007 || m_nErrNo == 8207 || m_nErrNo == 8407 || m_nErrNo == 8607 ||
		m_nErrNo == 4505 || m_nErrNo == 4506 || m_nErrNo == 4507 || m_nErrNo == 8008 || m_nErrNo == 8208 || m_nErrNo == 8408 || m_nErrNo == 8608 ||
		m_nErrNo == 8409 || m_nErrNo == 8609) {
		CString sCarID;
		m_stcCarrierID.GetWindowText(sCarID);
		if (sCarID.GetLength() < 4) {
			AfxMessageBox(_T("Carrier ID를 입력하세요...."));
			return;
		}
		if (m_nErrNo == 4005 || m_nErrNo == 4006 || m_nErrNo == 4007) { gData.sCarID_LoadStage[0] = sCarID; g_objSequenceMain.Set_MainRunCase(AUTO_LOAD_STAGE_1, 6); };
		if (m_nErrNo == 4505 || m_nErrNo == 4506 || m_nErrNo == 4507) { gData.sCarID_LoadStage[1] = sCarID;	g_objSequenceMain.Set_MainRunCase(AUTO_LOAD_STAGE_2, 6); };
		if (m_nErrNo == 8007 || m_nErrNo == 8008)					  { gData.sCarID_NGTray[0] = sCarID;	g_objSequenceMain.Set_MainRunCase(AUTO_NG_STAGE_1, 9);};
		if (m_nErrNo == 8207 || m_nErrNo == 8208)					  { gData.sCarID_NGTray[1] = sCarID;	g_objSequenceMain.Set_MainRunCase(AUTO_NG_STAGE_2, 9);};
		if (m_nErrNo == 8407 || m_nErrNo == 8408 || m_nErrNo == 8409) { gData.sCarID_GoodTray[0] = sCarID;	g_objSequenceMain.Set_MainRunCase(AUTO_GOOD_STAGE_1, 9);
																		gMes.nCarConfirm[1] = 1; g_objMesAgent.Set_CarrierIDReport("G", "", gData.sCarID_GoodTray[0]); };
		if (m_nErrNo == 8607 || m_nErrNo == 8608 || m_nErrNo == 8609) { gData.sCarID_GoodTray[1] = sCarID;	g_objSequenceMain.Set_MainRunCase(AUTO_GOOD_STAGE_2, 9);
																		gMes.nCarConfirm[1] = 1; g_objMesAgent.Set_CarrierIDReport("G", "", gData.sCarID_GoodTray[1]); };
	}
	if (m_nErrNo == 2416 || m_nErrNo == 2426 || m_nErrNo == 2805 || m_nErrNo == 2906 ||
		m_nErrNo == 2417 || m_nErrNo == 2427 || m_nErrNo == 2806 || m_nErrNo == 2907 ) {
		CString sMZID;
	...
	if (m_nErrNo == 4010 || m_nErrNo == 4510) gLot.nJobCycle = 1;
	if (m_nErrNo == 4011 || m_nErrNo == 4511) g_objInspector.Set_LotReady(TRUE);

	if (m_nErrNo == 9131 || m_nErrNo == 9132 || m_nErrNo == 2776) {
		g_objMesAgent.Set_LotEnd(gData.nLastPorNo, gData.sLastLotID, "0", "0", "0", "0", "0");
		gData.bInpectLotEnd = TRUE;
	}

	sLog.Format("[Error Mode] OK button push] (%d-%d-%s)", m_nErrNo, gLot.nJobCycle, gLot.sLotID[gLot.nJobCycle-1]);
	g_objLogFile.Save_HandlerLog(sLog);
	ShowWindow(SW_HIDE);
}
